...
{	
	...
	public function topMenu()
	{
		...
		return array(  
		    ...
		    'items'=>array(
                ...
                array('tag'=>"home",'label'=>'<i class="fa fa-map-marker"></i>'.Yii::t("default","View Restaurant by map"),
                'url'=>array('/store/map')),                                
             )   
          );
	}
	...
	{
		/*$menu[]=array('tag'=>"",'label'=>''.Yii::t("default","Terms and Conditions"),
                'url'=>array('/store/'));
        $menu[]=array('tag'=>"",'label'=>''.Yii::t("default","Privacy Policy"),
                'url'=>array('/store/'));*/

	   $menu=array();
       if ($data=Yii::app()->functions->customPagePosition("bottom")){
			foreach ($data as $val) {
				if ($val['is_custom_link']==2){
					if (!preg_match("/http/i", $val['content'])) {
						$val['content']="http://".$val['content'];
	...
	{
		$DbExt=new DbExt;
		$stmt="SELECT SUM(ratings) as ratings ,COUNT(*) AS count
		FROM
		{{rating}}
		WHERE
		merchant_id='".$merchant_id."'
	...
	public function addOnItemToArray($json_data='',$multi_option='',$multi_option_val,$two_flavors_position='',$require_addon='')
	{		
		$data='';
		$data_sub='';
		$json_data=!empty($json_data)?json_decode($json_data):false;		
		$this->data="list";
		$sub_category=$this->getSubcategory2();		
		//dump($sub_category);
	...
	public function getItemByCategory($category_id='',$addon=false)
	{
		$DbExt=new DbExt;
		$data='';
	...
	public function getItemById($item_id='',$addon=true)
	{
		...
		if ( $res=$DbExt->rst($stmt)){			
			foreach ($res as $val) {				
				...
				$cooking_ref=$this->cookingRefToArray($val['cooking_ref']);
                $addon_item=$this->addOnItemToArray($val['addon_item'],$multi_option,$multi_option_val,
                $two_flavors_position , $require_addon);				                
		...
		if ( $res=$this->getCategoryList2($merchant_id)){						
			foreach ($res as $cat_i=>$cat_name) {				
				$data[]=array(
				  ...
				  'dish'=>$cat_name['dish'],
				  'item'=>$this->getItemByCategory($cat_i)
				);
			}
			return $data;
		}
			          	 	...
			          	 	foreach ($val_sub as $addon_row=> $val_subs) {
			          	 		...
			          	 		if (isset( $val['addon_qty'][$cat_id] )){				          	 		    
			          	 		    $addon_qty=$val['addon_qty'][$cat_id][$addon_row];
			          	 	    	 ...
			          	 	    	 $addon_qty=$addon_qty*$qty; /** fixed addon qty */
...
'sub_item_name',$addon_name_trans)).'</div>';
    		    ...
    		    if ( !empty($tax)){
    				...
    				} else $taxable_subtotal=($subtotal+$delivery_charges+$merchant_packaging_charge)*$tax;    
...
$menu_html.="</li>";
    ...
    {    	
	    ...
	    $start_date=date('Y-m-d 00:00:00',strtotime($date_now . "-7 days"));
	...
	public function getBusinnesHours($merchant_id='')
	{
		...
		if (is_array($stores_open_day) && count($stores_open_day)>=1){
		   foreach ($stores_open_day as $days) {		   	  
		   	  ...
		   	  if (array_key_exists($days,$stores_open_ends)){		   	  
    ...
    public function getMerchantListOfPaymentGateway()
    {
		...
		if (!empty($paymentgateway)){
			...
			if ( $db_ext->rst($stmt)){
			   ...
			   $paymentgateway=array('paypal','stripe','mercadopago','ide','payu','pys');
			}
		}
