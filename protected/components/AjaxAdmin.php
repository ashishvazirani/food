    ...
    if (is_numeric($mt_delivery_miles)){    	
        			...
        			if (preg_match("/free-delivery/i",$_GET['filter_promo'])) {
						...
						if (is_array($stores_open_day) && count($stores_open_day)>=1){
							foreach ($stores_open_day as $val_open) {	
								...
								$tip.='<li><span>'.ucwords(Yii::t("default",$val_open)).'</span><value>'.$open_starts.
								" - ".$open_ends."&nbsp;&nbsp;&nbsp;".ucfirst($open_text).'</value></li>';
	    ...
	    public function setDeliveryOptions()
	    {
		   ...
		   if ( !Yii::app()->functions->isMerchantOpenTimes($merchant_id,$full_booking_day,$booking_time)){
				$this->msg=t("Sorry but we are closed on"." ".date("F,d l Y h:ia",strtotime($full_booking_time))).
	    	...
	    	if (is_array($order_item) && count($order_item)>=1){	
	    		...
	    		if ( Yii::app()->functions->code==1){
	    			...
	    			if (is_array($raw) && count($raw)>=1){	    				
	    				...
	    				/*FIXED ORDER STATUS*/	    					    				
if ($this->data['payment_opt']=="cod" || $this->data['payment_opt']=="pyr" || $this->data['payment_opt']=="ccr" ){
...
$params['cart_tip_value']=isset($this->data['cart_tip_value'])?$this->data['cart_tip_value']:'';                        			            
	    				...
	    				if ( $this->insertData("{{order}}",$params)){
		    			   ...
		    			   /*VOUCHER*/
		    				...
		    				} else if ($this->data['payment_opt'] =="stp") {
		    					$this->msg=Yii::t("default","Please wait while we connect you to Stripe.");	
		    				} else if ($this->data['payment_opt'] =="mcd") {
		    					$this->msg=Yii::t("default","Please wait while we connect you to Mercadopago.");
		    				} else if ($this->data['payment_opt'] =="ide") {
		    					$this->msg=Yii::t("default","Please wait while we connect you to Sisow.");	
		    				} else $this->msg=Yii::t("default","Your order has been placed.");*/
	    ...
	    {	    		    	
	    	...
	    	if ( $res=Yii::app()->functions->getReviewsList($this->data['merchant_id']) ){
	    		foreach ($res as $val) {	    			
	    			...
	    			$html.='<div id="#review-'.$val['id'].'" class="list-reviews rounded2">';
				     $html.='<div class="head">';
				        $html.='<div class="avatar left"><i class="fa fa-user"></i></div>';
				        $html.='<h4 class="left uk-text-bold">'.$val['client_name'].'</h4>';
				        $html.='<div class="clear"></div>';
				     $html.='</div>';
				     $html.='<div class="body">';
				       $html.='<p class="uk-text-muted uk-text-small">'.$pretyy_date.'</p>';
				       $html.='<p><span class="uk-text-bold">'.Yii::t("default",'Rated').'</span> 
				       <span class="uk-badge uk-badge-success">'.$val['rating'].'</span> '.$val['review'].'</p>';
				       
				       if ( $val['client_id']==$client_id ){
					       $html.='<div class="review-action right">';
					       $html.='<a data-id="'.$val['id'].'" href="javascript:;" class="edit-review uk-button">'.Yii::t("default","Edit").'</a>';
					       $html.='<a data-id="'.$val['id'].'" href="javascript:;" class="delete-review uk-button">'.Yii::t("default","Delete").'</a>';
					       $html.='</div>';
					       $html.='<div class="clear"></div>';
				       }
	    	...
	    	if ( $res=$DbExt->rst($stmt)){
	    		...
	    		foreach ($res as $val) {	    			
	    			...
	    			$feed_data['aaData'][]=array(
	    			  ...
	    			  prettyFormat($val['taxable_total'],$merchant_id),
	    			  prettyFormat($val['total_w_tax'],$merchant_id),
	    			  Yii::t("default",ucwords($val['status'])),
	    			  $date,
	    			  $action
	    		    );
	    		}
